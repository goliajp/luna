...
fn embedding_memory_cap_catches_runaway_alloc() {
    ...
    let v = vm
        .eval(
            "local ok, err = pcall(function () \
               local t = {} \
               for i = 1, 200000 do t[i] = {i, i, i, i} end \
               return t \
             end) \
             return ok, err",
        )
...
fn panic_string_native(_vm: &mut Vm, _fs: u32, _nargs: u32) -> Result<u32, luna::vm::LuaError> {
...
fn api_count_args(vm: &mut Vm, fs: u32, nargs: u32) -> Result<u32, luna::vm::LuaError> {
    Ok(vm.nat_return(fs, &[Value::Int(nargs as i64)]))
}

/// Native that returns each arg unchanged (multi-return passthrough).
fn api_echo(vm: &mut Vm, fs: u32, nargs: u32) -> Result<u32, luna::vm::LuaError> {
