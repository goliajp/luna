...
/// [`luna_jit_table_set_checked`]. It stays for the 3.x API.
// SAFETY: `no_mangle` is required for Cranelift's `Linkage::Import` to resolve this symbol from the JIT'd code; this crate is the sole producer of `luna_jit_*` symbols.
#[unsafe(no_mangle)]
// SAFETY: called only from Cranelift-emitted JIT code under an active JitVmGuard; the guard guarantees JIT_VM TLS holds a live &mut Vm for the dispatch window.
...
/// The trace JIT's table store, `t[key] = val`, for any key and value
/// the trace has a type for (both given as a `raw` tag and payload).
/// Returns `1` when stored. It stores nothing and returns `0` when the
/// table has a metatable, whose `__newindex` the helper would bypass, or
/// when the key cannot index a table (nil, NaN); the caller then
/// side-exits at the storing op and the interpreter performs it. The
/// trace goes no further than that op, so nothing it did before is
/// repeated.
// SAFETY: `no_mangle` is required for Cranelift's `Linkage::Import` to resolve this symbol from the JIT'd code; this crate is the sole producer of `luna_jit_*` symbols.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn luna_jit_table_set_checked(
    ...
    // SAFETY: the trace passes each tag with the payload of a register
    // of that kind, so tag and payload agree (see `runtime::value::raw`).
    let (key, val) = unsafe {
        (
            luna_core::runtime::Value::pack(
                key_tag as u8,
