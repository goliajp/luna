...
//! asserts all 30 `luna_jit_*` Cranelift trace-mcode helpers are
...
fn all_30_luna_jit_helpers_are_defined_in_staticlib() {
    ...
    // `crates/luna-jit/src/jit_backend/mod.rs`. If a 31st helper lands
    // upstream, grow this list AND the pin array AND the
    // `force_link_jit_helpers_reports_30` test in luna-runtime-helpers.
    let expected: [&str; 30] = [
        "luna_jit_new_table",
    ...
    assert!(
        missing.is_empty(),
        "staticlib `{}` is missing {} of 30 helper text symbols: {:?}\n\
