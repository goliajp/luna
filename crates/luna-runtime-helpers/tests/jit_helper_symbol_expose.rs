...
//! upval_get, …), the link step needs those 28 symbols to be present
//! as strong externs in the staticlib.
//!
//! This test pins that contract in two layers:
//!
//! 1. **Rust-level reachability** — each of the 28 helpers is
...
//!
//! 2. **`force_link_jit_helpers` returns 28** — confirms the
//!    `LUNA_AOT_HELPER_PIN` static is alive and indexes every helper
//!    once. A future PR that adds a 29th `pub unsafe extern "C" fn
...
/// The 28 helper symbols, re-checked via the crate's `pub use`
/// re-exports. Compile-fails if any name drifts out of upstream
/// `luna-jit::jit_backend`.
#[test]
fn all_28_helpers_reexported() {
...
/// "did we forget to grow the array when adding a 29th helper" gate.
#[test]
fn force_link_jit_helpers_reports_28() {
    let n = lrh::force_link_jit_helpers();
    assert_eq!(
        n, 28,
        "LUNA_AOT_HELPER_PIN must hold all 28 helper symbols; if you \
