...
//! upval_get, …), the link step needs those 29 symbols to be present
//! as strong externs in the staticlib.
//!
//! This test pins that contract in two layers:
//!
//! 1. **Rust-level reachability** — each of the 29 helpers is
...
//!
//! 2. **`force_link_jit_helpers` returns 29** — confirms the
//!    `LUNA_AOT_HELPER_PIN` static is alive and indexes every helper
//!    once. A future PR that adds a 30th `pub unsafe extern "C" fn
...
/// The 29 helper symbols, re-checked via the crate's `pub use`
/// re-exports. Compile-fails if any name drifts out of upstream
/// `luna-jit::jit_backend`.
#[test]
fn all_29_helpers_reexported() {
...
/// "did we forget to grow the array when adding a 30th helper" gate.
#[test]
fn force_link_jit_helpers_reports_29() {
    let n = lrh::force_link_jit_helpers();
    assert_eq!(
        n, 29,
        "LUNA_AOT_HELPER_PIN must hold all 29 helper symbols; if you \
