...
// v1.3 Stage 7 follow-on — re-export of the 34 `luna_jit_*` Cranelift
...
pub use luna_jit::jit_backend::{
    ...
    luna_jit_op_get_tab_up, luna_jit_op_tforcall, luna_jit_park_deopt, luna_jit_self_upval_check,
    ...
    luna_jit_str_buf_release, luna_jit_suppress_trace_admit, luna_jit_table_get_field,
    luna_jit_table_get_float, luna_jit_table_get_int, luna_jit_table_get_tagged,
    ...
    luna_jit_trace_materialize_frames, luna_jit_upval_get, luna_jit_upval_get_float,
...
mod jit_helpers_pin {
    ...
    static LUNA_AOT_HELPER_PIN: [PinnedFn; 34] = [
        ...
        PinnedFn(jb::luna_jit_table_get_tagged as AnyFn),
        PinnedFn(jb::luna_jit_upval_table_get_tagged as AnyFn),
    ...
    pub fn force_link_jit_helpers() -> usize {
        ...
        if NEVER_TRIP.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            unsafe {
                ...
                let _ = jb::luna_jit_table_get_tagged(0, 0, 0, std::ptr::null_mut());
                let _ = jb::luna_jit_upval_table_get_tagged(0, 0, std::ptr::null_mut());
...
/// v1.3 Stage 7 follow-on — pull all 34 `luna_jit_*` Cranelift
...
///
/// Returns the number of helper symbols pinned (always 34 with the
/// current `luna-jit` shape; will need to be bumped in lock-step
/// any time `crates/luna-jit/src/jit_backend/mod.rs` adds a 35th
