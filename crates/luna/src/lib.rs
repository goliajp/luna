...
// `use luna::vm::Vm`, `use luna::version::LuaVersion`,
// `use luna::runtime::Value`, etc. paths continue to resolve. The
...
// hangs off `luna::jit_backend` / the unified `luna::jit`
// re-export below.
pub use luna_core::*;
...
/// `use luna::jit::TraceRecord`, `use luna::jit::CraneliftBackend`,
/// `use luna::jit::cache_lookup_or_compile`, etc. resolve through
/// this module.
pub mod jit {
    ...
    // `luna::jit::cache_clear` only when both crates compile under
    ...
    /// `luna::jit_backend::trace` (the codegen entry points). Old
    /// `crate::jit::trace::TraceRecord` paths in user code keep
    /// working; new code can `use luna::jit::trace::try_compile_trace_with_options`.
    pub mod trace {
...
/// Bring it into scope with `use luna::VmExt;` to write
/// `vm.install_default_jit()` (matches v1.0 syntax) instead of
/// `luna::install_default_jit(&mut vm)`.
pub trait VmExt {
...
// level so `use luna::LuaVersion` works without an explicit
// `luna::version::LuaVersion` path.
