...
pub struct Vm {
    ...
    pub(crate) jit_pending_err: Option<LuaError>,
...
impl Vm {
    ...
    pub(crate) fn read_slot(&self, slot: u32, thread: Option<Gc<Coro>>) -> Value {
        let s = slot as usize;
    ...
    pub(crate) fn jit_last_lua_frame(&self) -> Option<Frame> {
    ...
    pub(crate) fn jit_ensure_stack(&mut self, need: usize) {
        if self.stack.len() < need {
            self.stack.resize(need, Value::Nil);
        }
    }
    ...
    pub(crate) fn jit_op_close(&mut self, start_offset: u32) -> i64 {
    ...
    pub(crate) fn jit_stack_update_raw(
        &mut self,
        slot_offset: u32,
        raw_bits: u64,
    ) {
    ...
    pub(crate) fn jit_str_buf_acquire(&mut self) -> *mut Vec<u8> {
    ...
    pub(crate) fn jit_str_buf_release(&mut self, buf: *mut Vec<u8>) {
        if buf.is_null() {
            return;
        }
    ...
    pub(crate) fn jit_str_buf_extend(
        &mut self,
        buf: *mut Vec<u8>,
        str_ptr: i64,
    ) -> i64 {
    ...
    pub(crate) fn jit_str_buf_intern(
        &mut self,
        buf: *mut Vec<u8>,
    ) -> i64 {
        if buf.is_null() {
    ...
    pub(crate) fn jit_stack_load(&mut self, slot_offset: u32) -> i64 {
        let Some(f) = self.jit_last_lua_frame() else {
            return 0;
        };
        let idx = (f.base as usize) + (slot_offset as usize);
    ...
    /// deopt for v2).
    pub(crate) fn jit_stack_tag(&mut self, slot_offset: u32) -> u8 {
        let Some(f) = self.jit_last_lua_frame() else {
    ...
    pub(crate) fn jit_push_inlined_frame(
        &mut self,
        cl: Gc<LuaClosure>,
        base: u32,
        pc: u32,
    ...
    pub(crate) fn find_or_create_upval(&mut self, slot: u32) -> Gc<Upvalue> {
    ...
    pub(crate) fn upval_get(&self, cl: Gc<LuaClosure>, idx: u32) -> Value {
