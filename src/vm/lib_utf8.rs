...
fn u_codepoint(vm: &mut Vm, fs: u32, nargs: u32) -> Result<u32, LuaError> {
    ...
    if i < 1 {
        return Err(arg_error(vm, 2, "codepoint", "out of bounds"));
    }
    if j > s.len() as i64 {
        return Err(arg_error(vm, 3, "codepoint", "out of bounds"));
    }
    let mut out = Vec::new();
...
fn u_len(vm: &mut Vm, fs: u32, nargs: u32) -> Result<u32, LuaError> {
    ...
    let end = j.clamp(0, s.len() as i64) as usize;
    while pos < end {
...
fn u_offset(vm: &mut Vm, fs: u32, nargs: u32) -> Result<u32, LuaError> {
    ...
    if !(1..=len as i64 + 1).contains(&i) {
        return Err(arg_error(vm, 3, "offset", "position out of bounds"));
    }
    let mut pos = (i - 1) as usize;
    if n == 0 {
        // start of the character containing byte i
        while pos > 0 && is_cont(s[pos]) {
            pos -= 1;
        }
        return Ok(vm.nat_return(fs, &[Value::Int(pos as i64 + 1)]));
    }
    ...
    if n > 0 {
        ...
        // 5.5: also return the final byte position of that character
        let mut end = pos;
        if end < len {
            end += 1;
            while end < len && is_cont(s[end]) {
                end += 1;
            }
        }
        return Ok(vm.nat_return(fs, &[Value::Int(pos as i64 + 1), Value::Int(end as i64)]));
    }
    // n < 0: walk backwards
    while n < 0 && pos > 0 {
        pos -= 1;
    ...
    Ok(vm.nat_return(fs, &[Value::Int(pos as i64 + 1), Value::Int(end as i64)]))
}
