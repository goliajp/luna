...
/// Find the first match of `pat` in `src` at or after `init` (byte offset).
/// `anchored_only` short-circuits the scan loop for `^` handled by caller.
pub fn find(src: &[u8], pat: &[u8], init: usize) -> Result<Option<Match>, PatError> {
    if init > src.len() {
        return Ok(None);
    }
    let (anchor, pat_body) = match pat.first() {
        Some(b'^') => (true, &pat[1..]),
        _ => (false, pat),
    };
    let mut s = init;
    loop {
        ...
        if let Some(e) = do_match(&mut st, s, 0)? {
            ...
            b'%' => match st.pat.get(*p + 1) {
                Some(b'b') => {
                    ...
                    let (Some(&x), Some(&y)) = (st.pat.get(*p + 2), st.pat.get(*p + 3)) else {
                        return Err(PatError("missing arguments to '%b'".into()));
                    };
                ...
                Some(&d @ b'0'..=b'9') => {
                    ...
                    if d == b'0' || idx >= st.caps.len() || st.caps[idx].1 < 0 {
