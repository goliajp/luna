...
use crate::runtime::Value as LuaValue;
...
/// Native entry-point produced by [`try_compile_int_chunk`] for a
/// chunk with zero params. Returns the chunk's `Return1` value as i64;
...
thread_local! {
    ...
    static JIT_VM: std::cell::Cell<*mut crate::vm::Vm> =
    ...
    /// `luna_jit_upval_get`).
    static JIT_CL: std::cell::Cell<*const crate::runtime::LuaClosure> =
        const { std::cell::Cell::new(std::ptr::null()) };
}

/// P11-S5c — RAII guard that pins the current `Vm` pointer in the
/// `JIT_VM` thread-local for the duration of a JIT call. The
/// dispatcher (`Vm::try_jit_call` / `Vm::try_jit_call_op`) acquires
/// one before transmuting & invoking the cached entry; drop on the
/// returned `JitVmGuard` clears the slot.
///
/// Re-entrance: a JIT'd self-recursive call (fib) re-enters the
/// dispatcher, which would set the same Vm pointer again — idempotent
/// because the Vm doesn't change. On the way out the inner guard
/// clears to null, then the outer guard's drop is a no-op. Safe as
/// long as luna is single-Vm-per-thread (which it is).
...
impl Drop for JitVmGuard {
    fn drop(&mut self) {
        ...
        // entry_fn, helpers fire inside entry_fn, returns to dispatcher
        ...
        //
        // Skip the 2 TLS writes per dispatch (~5-10 cycles each on arm64
        // thread_local fastpath). For fib_28 434k dispatches × ~10 cycles
        // = ~1.5 ms saved.
        //
...
/// guard whose drop clears the slot. Must be held across the JIT
/// entry-fn call so any helper can pick the pointer up.
///
...
pub fn enter_jit(
    vm: &mut crate::vm::Vm,
    cl: Option<crate::runtime::Gc<crate::runtime::LuaClosure>>,
) -> JitVmGuard {
    JIT_VM.with(|cell| cell.set(vm as *mut crate::vm::Vm));
    let cl_ptr = cl
        .map(|c| c.as_ptr() as *const crate::runtime::LuaClosure)
...
#[inline]
unsafe fn current_jit_vm<'a>() -> &'a mut crate::vm::Vm {
...
#[inline]
unsafe fn current_jit_closure() -> crate::runtime::Gc<crate::runtime::LuaClosure> {
    let p = JIT_CL.with(|cell| cell.get());
    debug_assert!(
        !p.is_null(),
        "luna_jit_upval_get called outside an upval-aware enter_jit scope"
    );
    crate::runtime::Gc::from_ptr(p as *mut crate::runtime::LuaClosure)
}
...
///
/// `kind` byte uses the same `crate::runtime::value::raw::*` tag
...
pub unsafe extern "C" fn luna_jit_materialize_sunk_table(
    ...
    if cap_u > 0 {
        for i in 0..cap_u {
            ...
            let v = unsafe { crate::runtime::Value::pack(kind, raw) };
            let _ = table.set_int(
                &mut vm.heap,
                (i + 1) as i64,
                v,
    ...
    if n_hash_u > 0 {
        for i in 0..n_hash_u {
            ...
            let key_gc: crate::runtime::Gc<crate::runtime::LuaStr> =
                crate::runtime::Gc::from_ptr(key_ptr_bits as *mut crate::runtime::LuaStr);
            let raw =
                crate::runtime::value::RawVal { zero: raw_bits };
            let v = unsafe { crate::runtime::Value::pack(kind, raw) };
            let _ = table.set(
                &mut vm.heap,
                crate::runtime::Value::Str(key_gc),
                v,
            );
        }
    }
...
pub unsafe extern "C" fn luna_jit_table_set_int(t: i64, key: i64, val: i64) {
    ...
    let g: crate::runtime::Gc<crate::runtime::Table> =
        crate::runtime::Gc::from_ptr(t as *mut crate::runtime::Table);
    ...
    let table = unsafe { g.as_mut() };
    let _ = table.set_int(&mut vm.heap, key, crate::runtime::Value::Int(val));
}
...
pub unsafe extern "C" fn luna_jit_table_set_raw(
    ...
    let v = unsafe {
        crate::runtime::Value::pack(
            tag as u8,
            crate::runtime::value::RawVal { zero: raw_bits as u64 },
        )
    };
...
pub unsafe extern "C" fn luna_jit_table_set_field(
    ...
    let key_gc: crate::runtime::Gc<crate::runtime::LuaStr> =
        crate::runtime::Gc::from_ptr(key_ptr as *mut crate::runtime::LuaStr);
    let key = crate::runtime::Value::Str(key_gc);
    let table = unsafe { g.as_mut() };
    let v = unsafe {
        crate::runtime::Value::pack(
            val_tag as u8,
            crate::runtime::value::RawVal { zero: val_raw as u64 },
        )
    };
...
pub unsafe extern "C" fn luna_jit_table_get_field(
    ...
    let key_gc: crate::runtime::Gc<crate::runtime::LuaStr> =
        crate::runtime::Gc::from_ptr(key_ptr as *mut crate::runtime::LuaStr);
    let v = g.get(crate::runtime::Value::Str(key_gc));
...
pub unsafe extern "C" fn luna_jit_table_set_nil(t: i64, key: i64) {
    ...
    let table = unsafe { g.as_mut() };
    let _ = table.set_int(&mut vm.heap, key, crate::runtime::Value::Nil);
}
...
pub unsafe extern "C" fn luna_jit_table_set_float_float(t: i64, key_bits: i64, val_bits: i64) {
    ...
    let table = unsafe { g.as_mut() };
    let k = crate::runtime::Value::Float(f64::from_bits(key_bits as u64));
    let v = crate::runtime::Value::Float(f64::from_bits(val_bits as u64));
...
pub unsafe extern "C" fn luna_jit_op_closure(proto_idx: i64) -> i64 {
    use crate::runtime::function::{INLINE_UPVALS_N, Upvalue, UpvalState};
    let vm = unsafe { current_jit_vm() };
    if vm.jit_pending_err.is_some() {
        return 0;
    }
    ...
    let mut stack_buf: [std::mem::MaybeUninit<crate::runtime::Gc<Upvalue>>;
        INLINE_UPVALS_N] = [std::mem::MaybeUninit::uninit(); INLINE_UPVALS_N];
    let mut heap_buf: Vec<crate::runtime::Gc<Upvalue>> = Vec::new();
    ...
    let v51 = vm.version() <= crate::version::LuaVersion::Lua51;
    ...
    let ups_slice: &[crate::runtime::Gc<Upvalue>] = ups;
    let nc = if v51 {
    ...
    let (_tag, raw) = crate::runtime::Value::Closure(nc).unpack();
    unsafe { raw.zero as i64 }
}
...
pub unsafe extern "C" fn luna_jit_trace_materialize_frames(
    n: u64,
    metas: *const crate::jit::trace::FrameMaterializeInfo,
) -> i64 {
    let vm = unsafe { current_jit_vm() };
...
/// S2c — entry-point shapes for compiled Protos with `num_params > 0`.
/// All args + return are `i64`; the dispatch layer picks the right
...
/// Maximum `num_params` count the lowerer accepts. Higher-arity
/// Protos bail to the interpreter. Bumping this is mechanical
/// (add a new `IntFnN` alias + match arm); 4 covers fib + every
/// common Lua-shaped numeric workload.
...
pub(crate) const TABLE_ARRAY_PTR_OFFSET: usize =
    std::mem::offset_of!(crate::runtime::Table, array_ptr);
pub(crate) const TABLE_ASIZE_OFFSET: usize = std::mem::offset_of!(crate::runtime::Table, asize);
...
pub(crate) const TABLE_METATABLE_OFFSET: usize =
    std::mem::offset_of!(crate::runtime::Table, metatable);
const RAW_TAG_INT: i64 = crate::runtime::value::raw::INT as i64;
const RAW_TAG_FLOAT: i64 = crate::runtime::value::raw::FLOAT as i64;
const RAW_TAG_TABLE: i64 = crate::runtime::value::raw::TABLE as i64;
const RAW_TAG_NIL: i64 = crate::runtime::value::raw::NIL as i64;

const _: () = {
    assert!(std::mem::size_of::<*mut u8>() == 8);
    assert!(std::mem::size_of::<crate::runtime::value::RawVal>() == 8);
    assert!(std::mem::align_of::<crate::runtime::value::RawVal>() == 8);
...
mod s2 {
    ...
    fn second_call_hits_cached_native() {
        ...
        assert_eq!(crate::jit::cache_entry_count(), 1, "one compiled Proto");
    }
...
mod s2c_a {
    ...
    fn with_inner<F: FnOnce(&crate::runtime::function::Proto)>(src: &str, f: F) {
        let mut vm = Vm::new(LuaVersion::Lua55);
        ...
        with_inner(
            ...
            |proto| {
                let handle = try_compile_int_chunk(
                    crate::runtime::Gc::from_ptr(proto as *const _ as *mut _),
                    false,
                    false,
                )
...
mod s2c_b {
    ...
    fn multiple_calls_share_cache() {
        ...
        assert_eq!(crate::jit::cache_entry_count(), 1, "Proto compiled exactly once");
    }
    ...
    fn jit_failed_state_falls_through() {
        ...
        assert_eq!(
            crate::vm::Vm::new(LuaVersion::Lua55)
...
mod s2c_c {
    ...
    fn recursive_one_compile_per_proto() {
        ...
        assert_eq!(crate::jit::cache_entry_count(), 1, "fib's Proto compiled exactly once");
    }
}
...
mod s3 {
    ...
    fn cache_key_includes_consts() {
        ...
        assert_eq!(crate::jit::cache_entry_count(), 2);
    }
...
mod s5a {
    ...
    fn cache_pre53_post53_distinct() {
        ...
        let mut vm55 = Vm::new(LuaVersion::Lua55);
        ...
        let mut vm53 = Vm::new(LuaVersion::Lua53);
...
mod s5b {
    ...
    fn math_libcall_distinct_fns_distinct_cache() {
        ...
        let n_after_sin = crate::jit::cache_entry_count();
        let _ = eval_float_with(
            LuaVersion::Lua55,
            "local s = 0.0 for i = 1, 4 do s = s + math.cos(i) end return s",
        );
        let n_after_cos = crate::jit::cache_entry_count();
        assert!(
